...
# không bao gồm lớp Fullyconnected
...
class ModelAI(nn.Module):
    ...
    def forward(self, x):
        x = self.backbone(x)
        return self.fcs(torch.flatten(x, start_dim=1))# bo qua chieu thu 1
    ...
    def _create_fullyconnection(self, dataSize, layers):
        return nn.Sequential(
            ...
            nn.Dropout(0.0),
            nn.Sigmoid(),
            nn.Linear(dataSize * dataSize * layers, 1)
        )
# def test():
#     model = ModelAI(in_channels=1, dataSize = 4, layers = 128)
#     x = torch.randn(2, 1, 13, 13)
#     # đầu ra của model (batch_size, 1)
#     print(model(x))
# test()